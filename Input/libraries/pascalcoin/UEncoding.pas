...
var
  ...
  i, LOffset: Int32;
begin
//  Result := False; // value assigned never used.
...
class function TPASCEncoding.TryDecode(const APasc: String;  out AMolinas: Int64): Boolean;
var
...
begin
  ...
  try
    LPosThousand := LMoneyString.IndexOf( FormatSettings.ThousandSeparator );
    LPosDecimal  := LMoneyString.IndexOf( FormatSettings.DecimalSeparator );

    if (LPosThousand>0) then begin
      if (LPosThousand < LPosDecimal ) then begin
        // Remove thousand values
        LMoneyString := LMoneyString.Replace(String( FormatSettings.ThousandSeparator),'',[rfReplaceAll]);
      end else begin
        // Possible 15.123.456,7890 format ( coma (,) = decimal separator )
        // Remove decimal "." and convert thousand to decimal
        LMoneyString := LMoneyString.Replace(String( FormatSettings.DecimalSeparator),'',[rfReplaceAll]);
        LMoneyString := LMoneyString.Replace( FormatSettings.ThousandSeparator, FormatSettings.DecimalSeparator,[rfReplaceAll]);
      end;
    end;

    AMolinas := Round( StrToFloat(LMoneyString, FormatSettings)*10000 );
    Result := true;
  Except
    result := false;
  end;
...
begin
  Result := FormatFloat('#,###0.0000',(AMolinas/10000.0), FormatSettings);
end;
